...
# --- Isaac Sim boot ---
from isaacsim import SimulationApp  # bleibt gleich in beiden Welten
...
# Isaac/Omniverse (kompatibel zu alten & neuen Namensräumen)
try:
    # Ältere/Legacy-Namespace
...
class IsaacStackingRunner:
    ...
    def _ensure_material_pool(self, stage, named_rgba_list, mat_root="/World/Looks"):
        return [self._get_or_create_color_material(stage, name, rgba, mat_root) for name, rgba in named_rgba_list]
    ...
    def _sample_points_in_front_rectangle_local(
        ...
        assert n >= 1 and width > 0.0 and length > 0.0 and base_clearance >= 0.0
    ...
    def _randomize_stacking_in_rectangle_existing_task(
        ...
        for i, name in enumerate(cube_names):
            ...
            if randomize_rotation and not keep_cubes_rot:
                ...
                if rotation_mode == "yaw":
                    ...
                    yaw_deg = float(local_rng.uniform(*yaw_range_deg))
                    q_delta = rot_utils.euler_angles_to_quats(
        ...
        task.set_params(stack_target_position=np.array([target_local[0], target_local[1], 0.0], dtype=float))
        ...
        def _log(tasks, scene):
            ...
            return {
                ...
                "applied_joint_positions": (
                    aa.joint_positions.tolist() if aa and aa.joint_positions is not None else []
                ),
            }
        return _log
                ...
                if self.world.is_playing():
                    if reset_needed:
                        ...
                        if n >= 1:
                            ...
                            try:
                                print(
                                    f"[DataLogger] frames={n} peek@{idx}:",
                                    self.logger.get_data_frame(data_frame_index=idx),
                                )
...
def parse_args():
    ...
    p.add_argument("--logdir", type=Path, default=Path("./00_my_envs/Franka_Cube_Stacking_JW/logs"))
