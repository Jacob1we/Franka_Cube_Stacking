...
FRANKA_BASE_CLEARANCE = 0.30 # 20 cm
...
def add_or_update_allowed_area_plane(
    ...
    bind_red_material=True,
    material_pool_named_rgba=None,  # <- NEU: Liste von (name, rgba)-Tuples
    material_seed=None              # <- NEU: Seed für deterministische Auswahl
):
    """
    ...
    elif bind_red_material:
        mat = _get_or_create_color_material(stage, name="AllowedArea_Red", rgba=(1.0,0.0,0.0,0.5))
...
def sample_points_in_front_rectangle_local(
    ...
    base_clearance= FRANKA_BASE_CLEARANCE,        # float: **NEU** Mindest-XY-Abstand zur *Basis* (Sicherheitszone), in m
...
def build_world(cam_freq: int, cam_res: tuple[int, int]):
    ...
    ALLOWED_AREA_MATS = [
        ("AllowedArea_Red",     (1.00, 0.05, 0.10, 0.40)),
        ("AllowedArea_Green",   (0.10, 0.85, 0.25, 0.30)),
        ("AllowedArea_Blue",    (0.10, 0.45, 0.95, 0.35)),
        ("AllowedArea_Orange",  (1.00, 0.55, 0.10, 0.35)),
        ("AllowedArea_Purple",  (0.55, 0.25, 0.85, 0.35)),
        ("AllowedArea_Yellow",  (1.00, 0.95, 0.10, 0.30)),
        ("AllowedArea_Teal",    (0.10, 0.85, 0.80, 0.35)),
    ]
